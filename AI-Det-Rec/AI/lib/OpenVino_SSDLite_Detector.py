'''
@FileName   :OpenVino_SSDLite_Detector.py
@Description:openvino部署推理ssd
@Date       :2024/03/05 15:11:38
@Author     :daito
...
class OpenVinoSSDLiteDetector(object):
    def __init__(self, IN_conf):
        ...
        net = ie_core.compile_model(
            model=model, device_name=IN_conf.get("device"))
    ...
    def postprocess_results(self, h, w, results, thresh=0.6):
    ...
    def detect(self, image):
        ...
        input_img = cv2.resize(
            src=image, dsize=(self.INPUT_WIDTH,
                              self.INPUT_HEIGHT), interpolation=cv2.INTER_AREA
        )
        ...
        h, w = image.shape[:2]
        class_ids, scores, boxes = self.postprocess_results(
            h, w, results=results)
