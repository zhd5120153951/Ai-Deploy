'''
@FileName   :OpenVino_Yolov5_Detector.py
@Description:openvino框架部署onnx
@Date       :2024/03/05 15:06:13
@Author     :daito
...
# the version of openvino <= 2021.4.2
...
class OpenVinoYoloV5Detector():
    ...
    def __init__(self, IN_conf):
        ...
        self.Net = ie.load_network(network=IN_conf.get(
            "weight_file"), device_name=IN_conf.get("device"))
    ...
    def postprocess_results(self, h, w, results, thresh=0.25):
        class_ids = []
        boxes = []
        scores = []
    ...
    def detect(self, image):
        ...
        # results = results["output0"]#yolov5s.onnx是output0
        results = results["output"]  # 非onnx不是

        h, w, _ = input_img.shape
        # h,w, _ = image.shape
        class_ids, scores, boxes = self.postprocess_results(h, w, results)
...
if __name__ == '__main__':
    ...
    IN_conf = {
        ...
        "weight_file": "../weights/yolov5s.onnx",
        "device": "GPU"
    }
    ...
    while True:
        ...
        if r:
            ...
            cv2.imshow('OpenVino_Yolov5_Detector.py', frame)
            cv2.waitKey(1)
