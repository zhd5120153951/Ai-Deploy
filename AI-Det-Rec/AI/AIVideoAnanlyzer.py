'''
@FileName   :AIVideoAnanlyzer.py
@Description:AI端
@Date       :2024/03/20 16:02:11
@Author     :daito
...
@app.route("/stream/preview", methods=["POST"])
def streamPreview():
    # 前端传来地址,这里返回取到的流
    data = {
        "code": 0,
...
@app.route('/stream/controlAdd', methods=['POST'])
def streamControlAdd():
    data = {
        "code": 0,
        "msg": "unknown error",
    }
