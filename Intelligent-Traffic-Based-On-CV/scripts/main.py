...
class Main(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def display_video(self):
        self.openFIleButton.setEnabled(False)  # 推理时打开不可用
        self.closeFileButton.setEnabled(True)  # 关闭可用
    # RGB转BGR
        frames = 0
                ...
                if frames % self.changeFrameSlider.value() == 0:
                    ...
                    try:
                        ...
                        if self.cars_detect.isChecked():
                            ...
                            self.tableWidget.setItem(
                                0, 1, QTableWidgetItem(str(0)))
                            self.tableWidget.setItem(
                                0, 2, QTableWidgetItem(str(0)))
                            # print(1111)
                        ...
                        if self.license_plate_detect.isChecked():
                            ...
                            for plate_info in plate_info_list:
                                ...
                                plate_center = [
                                    int(rect[0] + rect[2]), int(rect[1] + rect[3])]  # 车牌中心中心位置
    ...
    def load_model(self):
        ...
        load_completed = QMessageBox.information(
            self, 'message', '模型加载完成．', QMessageBox.Ok)
