...
def traffic_light_detect(x, img):
    for i in range(x.shape[0]):
        ...
        if cls == 9:
            img_copy = img.copy()
            traffic_light_roi = img_copy[int(x[i, 2]):int(
                x[i, 4]), int(x[i, 1]):int(x[i, 3])]
...
def write(output, img, motors_center):
    ...
    for x in output:
        ...
        colors = [[225, 0, 0], [0, 225, 0], [0, 0, 225], [
            225, 225, 225], [205, 90, 106]]       # blue green red black
        cls = int(x[-1])
        ...
        if cls == 9:
            img_copy = img.copy()
            traffic_light_roi = img_copy[int(
                x[2]):int(x[4]), int(x[1]):int(x[3])]
...
def target_detect(model, frame):
    ...
    motors_center = get_motor_poisition(
        output)  # 获得在人行道上行驶的摩托车的中心坐标,用于检测行人是否违规
