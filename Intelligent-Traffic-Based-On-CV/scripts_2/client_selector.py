...
class Main(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def __init__(self):
        ...
        # 设置窗口位于屏幕中心
        self.function_dict = {
            'target_detect_is_open': False, 'traffic_light_detect_is_open': False,
            'cars_detect_is_open': False,
            'people_detect_is_open': False,
        ...
        self.subWinThread = threading.Thread(
            target=self.subWinFunc, daemon=True)
        self.subWinThread.start()

        # 加载日志
        self.receiveLogSignal.connect(lambda log: self.logOutput(log))
        self.logOutputThread = threading.Thread(
            target=self.receiveLog, daemon=True)
    ...
    def connectBox(self):
        load_completed = QMessageBox.information(
            self, 'message', '成功连接到服务器', QMessageBox.Ok)

    def clickConnect(self):
        if not self.socket_state:
            # 如果是之前是断开状态(close),则按钮显示是`connect`,需要将按钮改为`close`
            self.pushButton.setText("close")
            self.client_tcp_thread = threading.Thread(
                target=self.connectServer, daemon=True)
    ...
    def connectServer(self):
        ...
        while True:
            ...
            if self.target_detect.isChecked():
                current_state = 1
                # self.rtmp_deal_address = "rtmp://101.132.236.124/live/stream" #origin
                self.rtmp_deal_address = "rtsp://admin:jiankong123@192.168.23.15:554/Streaming/Channels/101"  # zhd
                ...
                if self.cars_detect.isChecked():
                    ...
                    self.tableWidget.setItem(
                        0, 1, QTableWidgetItem(str(result_dict['cars_num'])))
            ...
            if (result_dict['pedestrians_num']):
                break_info_log = str(
                    result_dict['pedestrians_num']) + "人闯红灯;\n"
