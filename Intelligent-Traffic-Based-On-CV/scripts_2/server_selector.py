...
class ServerAchieve:
    def __init__(self):
        ...
        self.server_tcp_thread = threading.Thread(
            target=self.connectClient, daemon=True)
        self.server_tcp_thread.start()

        # self.cap = cv2.VideoCapture("rtmp://101.132.236.124/live/livestream")#origin code
        self.cap = cv2.VideoCapture(
            "rtsp://admin:jiankong123@192.168.23.15:554/Streaming/Channels/101")  # zhd
        ...
        # 处理结果多进程队列
        self.deal_data_queue = mp.Queue(maxsize=10)

        video_process = [
            mp.Process(target=self.readVideo),
            mp.Process(target=self.dealVideo,),
            mp.Process(target=self.dealData,)
        ]
    ...
    def dealVideo(self):
        ...
        data_dict = {
            'frame': None, 'output': None, 'pedestrians_num': None, 'plate_info_list': []
        }
        ...
        while True:
            ...
            output, orign_img, frame, pedestrians_num = target_detect(
                model, frame)
    ...
    def socketAccept(self, sock, mask):
        ...
        self.mysel.register(
            new_connection, selectors.EVENT_READ, self.socketRead)
