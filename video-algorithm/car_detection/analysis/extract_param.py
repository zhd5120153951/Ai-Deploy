def param(data):
    ...
    if data['common_args']['polyline'] == 'True':
        ...
        polyline = False
    conf_thres_car = float(data['model_args']['conf_thres_car'])
    iou_thres_car = float(data['model_args']['iou_thres_car'])
    line_thickness = int(data['common_args']['line_thickness'])

    return detect_area_flag, hide_labels, hide_conf, polyline, conf_thres_car, iou_thres_car, line_thickness