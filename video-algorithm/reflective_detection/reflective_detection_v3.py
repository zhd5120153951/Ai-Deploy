...
from analysis.reflective import process_result_reflective
...
def run_analysis(data, update = False):
    ...
    # 图片预处理
    pred_person, pred_reflective, names_person, names_reflective, img, img_array, degree = img_preprocess(device, data, MODEL_person, MODEL_reflective, engine_person, engine_reflective, params)

    if len(pred_person) == 0 and len(pred_reflective) == 0:
    ...
    head_helmet = {}
    # ------------- #
    img_array_copy = process_result_reflective(pred_reflective, names_reflective, data, img, img_array, person, head_helmet, params)

    if update:
        for weights in [weights_person_path, weights_reflective_path]:
...
def load_model():
    global device
    global MODEL_person
    global MODEL_reflective
    global engine_person
    global engine_reflective

    MODEL_person = attempt_load(weights_person_path, device)  # load FP32 model
    MODEL_reflective = attempt_load(weights_reflective_path, device)

    # engine_person, engine_reflective = load_engine(engine_person_path, engine_reflective_path)
...
weights_reflective_path = 'weights/reflective.pt'
engine_person_path = './weights/helmet_head_person_l.engine'
engine_reflective_path = './weights/reflective.engine'
MODEL_person = None
MODEL_reflective = None
engine_person = None
engine_reflective = None
