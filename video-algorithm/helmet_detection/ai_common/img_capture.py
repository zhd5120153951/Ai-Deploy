...
def cap_rtsp(json, job_id):
    try:
        response = requests.get(setting.SERVICE_PATH + f"/rtsp/get/{json['k8sName']}")
        response.content.replace(bytes('\\', encoding = 'utf-8'), bytes('/', encoding = 'utf-8'))
        logger.debug(f"设备{json['k8sName']},请求状态为{response},请求返回内容为{response.content}")
...
def cap_img(json_list, job_id, k8sName_list, device_list):
    try:
        response = requests.post(setting.SERVICE_PATH + '/image_base64/get/list', json=k8sName_list)
    except Exception as e:
        logger.info(f'Job_id{job_id}failed to connect to the image agent service. Send the following exception information to the alarm service{e}.')
        alarm_dic = package_code_exception(f'Job_id{job_id}failed to connect to the image agent service, exit the analysis and perform the next check. Exception information is{e}.', 101, device_list[0])
        setting.alarm_queue.put(list(alarm_dic))
        raise Exception(f'Job_id{job_id}failed to connect to the image agent service. Exit the analysis and perform the next check.')
    else:
        res = response.json()
        if res['code'] == 0:
            res_data = res['data']
            ...
            for json in json_list:
                ...
                logger.debug(f"设备 {json['k8sName']} 检测图片张数为 {setting.NUM[json['k8sName']]}")
            logger.info(f'Job_id{job_id}get images successfully. The return message is{res_msg}, Return code is{res_code}.')
        else:
            alarm_dic = package_code_exception(
            ...
            setting.alarm_queue.put(list(alarm_dic))
            raise Exception(f'Job_id{job_id}failed to get images. Error code is{response.status_code}. Exit the analysis and perform the next check.')


def cap_rtsp_native(json_list, rtsp_ai_name, ai_name):
    capture = cv2.VideoCapture(setting.video_path)
...
def cap_data(json_list, job_id):
    ...
    if setting.TEST_FLAG == 0:
        ...
        # 请求图片服务代理
        logger.info(f"Job_id{job_id}start requesting the image service agent:{setting.SERVICE_PATH}. The contained device list is {k8sName_list}.")
