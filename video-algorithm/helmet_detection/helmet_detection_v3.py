...
def run_analysis(data, update=False):
    detect_area_flag, hide_labels, hide_conf, polyline, conf_thres, iou_thres, line_thickness = param(
        data)

    # 图片预处理
    pred, names, img, img_array, degree = img_preprocess(
        device, data, conf_thres, iou_thres, MODEL, engine)
    if len(pred) == 0:
    ...
    # 安全帽检测
    img_array_copy = process_result_helmet(
        pred, detect_area_flag, names, hide_labels, hide_conf, line_thickness, data, img, img_array, polyline)
