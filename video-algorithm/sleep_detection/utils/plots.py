...
class Annotator:
    ...
    def masks(self, masks, colors, im_gpu=None, alpha=0.5):
        ...
        if im_gpu is None:
            ...
            s = masks.sum(2, keepdims=True).clip(
                0, 1)  # add all masks together
            ...
            colors = torch.tensor(
                colors, device=im_gpu.device, dtype=torch.float32) / 255.0
...
def plot_images(images, targets, paths=None, fname='images.jpg', names=None):
    ...
    for i in range(i + 1):
        ...
        if paths:
            annotator.text(
                (x + 5, y + 5), text=Path(paths[i]).name[:40], txt_color=(220, 220, 220))  # filenames
        if len(targets) > 0:
...
# 绘制框 --zhd 2024/09/04--新版本的v5去掉了plot_one_box函数


def plot_one_box(x_, img_array_copy, label=None, color=(
        0, 0, 255), line_thickness=1):
    # Plot one bounding box on image
    tl = line_thickness or round(
        0.002*(img_array_copy.shape[0]+img_array_copy.shape[1])/2)+1  # line thickness font
    color = color or [random.randint(0, 255) for _ in range(3)]
    c1, c2 = (int(x_[0]), int(x_[1])), (int(x_[2]), int(x_[3]))
    cv2.rectangle(img_array_copy, c1, c2, color, tl, lineType=cv2.LINE_AA)
    if label:
        tf = max(tl-1, 1)
        t_size = cv2.getTextSize(label, 0, fontScale=tl/3, thickness=tf)[0]
        c2 = c1[0]+t_size[0], c1[1]-t_size[1]-3
        cv2.rectangle(img_array_copy, c1, c2, color, -1, cv2.LINE_AA)  # filled
        cv2.putText(img_array_copy, label, (c1[0], c1[1]-2), 0, tl/3,
...
def save_gas(img, file=Path('im.jpg'), save=True):
    if save:
        ...
        f = str(increment_path(file).with_suffix('.jpg'))
        # cv2.imwrite(f, crop)  # save BGR, https://github.com/ultralytics/yolov5/issues/7007 chroma subsampling issue
        Image.fromarray(img).save(f, quality=95, subsampling=0)  # save RGB
