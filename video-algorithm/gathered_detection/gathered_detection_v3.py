...
from analysis.gathered import process_result_gathered
...
def run_analysis(data, update=False):
    detect_area_flag, hide_labels, hide_conf, polyline, conf_thres_gathered, iou_thres_gathered, line_thickness = param(data)

    # 图片预处理
    pred_gathered, names_gathered, img, img_array, degree = img_preprocess(device, data, conf_thres_gathered, iou_thres_gathered, MODEL_gathered, engine)
    if len(pred_gathered) == 0:
        logger.info(f"设备 {data['k8sName']} 相似帧过滤")

        return img_array

    # 人员聚集检测
    img_array_copy = process_result_gathered(pred_gathered, detect_area_flag, names_gathered, hide_labels, hide_conf, line_thickness, data, img, img_array, polyline)

    # update model (to fix SourceChangeWarning)
    if update:
        strip_optimizer(weights_gathered)
...
def load_model():
    global device
    global MODEL_gathered
    global engine

    MODEL_gathered = attempt_load(weights_gathered, device)  # load FP32 model
    engine = load_engine(engine_path)



device = select_device('')
weights_gathered = './weights/helmet_head_person_l.pt'
