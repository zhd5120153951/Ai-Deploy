...
def upload():
    if request.method == 'POST':  # post是一种请求方式
        ...
        # 保存到upload中--将文件保存到服务器本地
        # 本地路径+图片名字= 文件路径（file-path)
        global upload_file_path
        upload_file_path = os.path.join(os.getcwd(), 'upload')
        if not os.path.exists(upload_file_path):
            os.makedirs(upload_file_path)
            print(f"Directory upload has been created.")
        else:
            print(f"Directory upload already exists.")

        # f.save(file_path)  # 保存上传的图片到本地目录下，方便后续推理，直接找到图片
        f.save(os.path.join(upload_file_path, filename))
        # 返回文件路径
        # return file_path
...
def parse_opt():
    ...
    parser.add_argument('--source', type=str, default=upload_file_path,
                        help='file/dir/URL/glob/screen/0(webcam)')
    parser.add_argument('--data', type=str, default='models/yolov5s.yaml',
